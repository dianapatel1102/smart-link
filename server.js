...
async function connectDB() {
  ...
  if (!admin) {
    await usersCol.insertOne({ username: "admin", password: "admin123" });
  }
  console.log("✅ MongoDB Connected! Data permanent hai!");
}
connectDB();
...
app.post("/api/create", auth, async (req, res) => {
  ...
  await analyticsCol.insertOne({
    code, total: 0, humans: 0, bots: 0,
    devices: {}, browsers: {}, os: {},
    hourlyClicks: {}, dailyClicks: {}, logs: []
  });
...
app.get("/:code", async (req, res) => {
  ...
  const updateFields = {
    $inc: { total: 1, [isBot ? "bots" : "humans"]: 1 },
    $push: { logs: { $each: [{ type: isBot ? "bot" : "human", time: new Date() }], $slice: -100 } }
  };
  
  if (!isBot) {
    const device = getDevice(ua), browser = getBrowser(ua), os = getOS(ua);
    updateFields.$inc[`devices.${device}`] = 1;
    updateFields.$inc[`browsers.${browser}`] = 1;
    updateFields.$inc[`os.${os}`] = 1;
    updateFields.$inc[`hourlyClicks.${hour}`] = 1;
    updateFields.$inc[`dailyClicks.${day}`] = 1;
  }
  
  await analyticsCol.updateOne({ code: req.params.code }, updateFields);
...
app.get("/api/analytics/:code", auth, async (req, res) => {
  ...
  res.json({
    ...
    overview: {
      totalClicks: t, humanClicks: stats?.humans || 0, botClicks: stats?.bots || 0,
      humanRate: t > 0 ? Math.round((stats.humans/t)*100) : 0,
      botRate: t > 0 ? Math.round((stats.bots/t)*100) : 0
    },
...
// DELETE
app.delete("/api/delete/:code", auth, async (req, res) => {
  const link = await linksCol.findOne({ code: req.params.code });
  if (!link || link.owner !== req.user) return res.json({ error: "Not allowed" });
  await linksCol.deleteOne({ code: req.params.code });
  await analyticsCol.deleteOne({ code: req.params.code });
